...
async function extractPublishedDate(page) {
  try {
    ...
    if (url.includes('aptos-infrastructure')) {
      ...
      if (aptosDate) {
        console.log(`Found date for Aptos report via special handling: ${aptosDate}`);
        ...
        const aptosDateMatch = cleanDate.match(/([A-Za-z]{3})\s+(\d{1,2}),\s+(\d{4})/i);
        if (aptosDateMatch) {
        ...
        cleanDate = cleanDate.replace(/\bJAN\b/i, 'January')
    ...
    const publishedDate = await page.evaluate(() => {
      ...
      for (const selector of delphiSpecificSelectors) {
        try {
          ...
          // Extract date text
          const dateText = element.tagName === 'TIME' ? 
            (element.getAttribute('datetime') || element.textContent.trim()) : 
            element.textContent.trim();
          
          if (dateText) {
            console.log(`Found date via Delphi-specific selector '${selector}': ${dateText}`);
            return dateText;
          }
        } catch (error) {
          console.error(`Error with Delphi-specific selector ${selector}:`, error.message);
        }
      }
      ...
      for (const selector of secondarySelectors) {
        try {
          ...
          if (element.tagName === 'META') {
            ...
            dateText = element.getAttribute('datetime') || element.textContent.trim();
          } else if (element.getAttribute('itemprop') === 'datePublished') {
            dateText = element.getAttribute('content') || element.textContent.trim();
          } else {
            dateText = element.textContent.trim();
          }
          
          if (dateText) {
            console.log(`Found date via secondary selector '${selector}': ${dateText}`);
            return dateText;
          }
        } catch (error) {
          console.error(`Error with secondary selector ${selector}:`, error.message);
        }
      }
      
      // Last resort: Look for specific text patterns like "Published on {date}"
      try {
        const publishedOnPattern = /published\s+on\s+([a-zA-Z]+\s+\d{1,2},?\s+\d{4}|\d{1,2}\s+[a-zA-Z]+,?\s+\d{4}|\d{4}-\d{2}-\d{2})/i;
        const bodyText = document.body.innerText;
        const publishedMatch = bodyText.match(publishedOnPattern);
        
        if (publishedMatch && publishedMatch[1]) {
          console.log(`Found date via "Published on" pattern: ${publishedMatch[1]}`);
        ...
        const headerAndFirstParagraphs = Array.from(document.querySelectorAll('header, h1, h2, .article-header, p')).slice(0, 5).map(el => el.textContent).join(' ');
        
        for (const pattern of datePatterns) {
          const match = headerAndFirstParagraphs.match(pattern);
          if (match && match[0]) {
            console.log(`Found date via targeted pattern in header/first paragraphs: ${match[0]}`);
            return match[0];
          }
        }
      } catch (error) {
    ...
    if (publishedDate) {
      ...
      if (!isNaN(parsedDate)) {
        ...
        if (parsedDate > oneYearAgo && parsedDate < tenYearsInFuture) {
          ...
          console.warn(`Extracted date "${cleanDate}" parsed as ${parsedDate.toISOString()} is outside reasonable range`);
  ...
  for (const report of testReports) {
    try {
      ...
      const extractedDateString = extractedDate ? 
        (extractedDate.includes('T') ? extractedDate.split('T')[0] : extractedDate) : null;
